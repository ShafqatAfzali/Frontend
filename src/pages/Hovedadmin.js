...
export default function Hovedadmin(){
    ...
    async function logut(){
        const resp = await fetch("/api/logut",{
            method:"GET",
            headers:{
    ...
    return (auth ? <div>
        ...
        <br/>
        <Link to="/" onClick={logut}>
        <button style={{position:"relative", left:"90vw", bottom:"8vh", height:"6vh", width:"9vw", backgroundColor:"lightblue" }} >log ut</button>
        </Link>
        ...
        {visalt && (vis_hva.ikke_sendt && <div> 
            ...
            {kunde.map((person,index)=>{
                ...
                if(person.status_ === "ikke sendt"){
                    ...
                    <h3>status:{person.status_}, bestilling-tid:{person.tid_kjøpt} varer-kjøpt:{salget.produkt_kjøpt}</h3>
                    <input className="bekk" type="radio" value={[person.id,person.betaling_id,person.email,person.navn]} onClick={(e)=>{setendring([...endring,[e.target.value]])}}/>send
                </li>}
            })}
        </ul></div>)}
    ...
    </div> : <div><h1>ingen tilgang</h1></div> )
}