...
const stripePromise = loadStripe("pk_test_51MtsmhJntTzBTQrmKJG5NtjGCMsHxaWS3cUbBBRHELCA3sFdgPDk1uyt6KWRR2pap8VGJDL6200V05XT4g9WcXkD0083V92DEv");
    ...
    if (bought.length > 0) {
        return <div className="Side-cart" style={vis_cart ? (vekk ? tredje: første) : andre}>
            ...
            <div className='Selected'>
                {bought.map((element, index) => {
                    ...
                    return <div className='bought-elements' key={index}>
                        ...
                        <h6 className="bought-element-price_eur" >Price: {(valgt.pris/12).toFixed(1)} EUR</h6>
                        <h6 className='bought-element-total'>total(kr): {total} NOK</h6>
                        <h6 className='bought-element-total_eur'>total(eur): ~{(total/12).toFixed(1)} EUR</h6>
                    </div>
                })}
            </div>
            ...
            <div className="cart_bunn">
                ...
                <h3 className="total_eur">Total(eur): {(total/12).toFixed(1)} EUR</h3>
                <Link to="/kasse" ><div className="payment_btn" onClick={send} onMouseOver={()=>{sethover(true)}} onMouseLeave={()=>{sethover(false)}}>
